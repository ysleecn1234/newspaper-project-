...
"""
URL 기반 뉴스 기사 크롤러
 - 서울신문 기사 페이지를 크롤링하도록 특화된 셀렉터/패턴 반영
...
try:
    from C_seoul_database_manager import db_manager
    DB_AVAILABLE = True
except Exception as e:
    logger = logging.getLogger(__name__)
    logger.warning(f"서울신문 데이터베이스 매니저를 불러올 수 없습니다: {e}")
    db_manager = None
    DB_AVAILABLE = False
...
DEFAULT_CATEGORY_URLS: List[str] = [
    ...
    'https://www.seoul.co.kr/newsList/society/', #사회
    'https://www.seoul.co.kr/newsList/economy/', #경제
    'https://www.seoul.co.kr/newsList/international/', #국제
    'https://www.seoul.co.kr/newsList/life/', #문화/건강
    'https://www.seoul.co.kr/newsList/plan/', #기획/연재
    'https://www.seoul.co.kr/newsList/editOpinion/', #오피니언
    'https://www.seoul.co.kr/newsList/peoples/', #피플
...
class UrlArticleCrawler:
    ...
    def _extract_article_links(self, soup: BeautifulSoup, current_url: str) -> List[str]:
        # 서울신문 기사 링크 패턴 위주로 수집
        patterns = [
            # 서울신문 기사 URL 패턴: /news/newsView.php?id=숫자
            re.compile(r'/news(?:/)?newsView\.php\?id=\d+', re.I),
            # 일부 변형 패턴 호환
            re.compile(r'newsView\.php\?id=\d+', re.I),
        ]
        links: List[str] = []
        seen = set()
        for a in soup.find_all('a', href=True):
            ...
            # 서울신문 도메인 또는 상대경로 확인
            if 'seoul.co.kr' in href or href.startswith('/'):
                for pat in patterns:
                    if pat.search(href):
                        full = self._normalize_url(current_url, href)
                        if full and full not in seen:
                            # 서울신문 기사 URL인지 추가 검증
                            if self._is_seoul_article_url(full):
                                links.append(full)
                                seen.add(full)
                        break
        return links

    def _is_seoul_article_url(self, url: str) -> bool:
        """서울신문 기사 URL인지 확인"""
        try:
            parsed = urlparse(url)
            if 'seoul.co.kr' not in parsed.netloc:
                return False
            # 대표 기사 URL 패턴 확인: /news/newsView.php?id=숫자
            if re.search(r'/news(?:/)?newsView\.php\?id=\d+', parsed.path + ('?' + parsed.query if parsed.query else ''), re.I):
                return True
            return False
        except Exception:
            return False
    ...
    def _derive_category_name(self, category_url: str) -> Optional[str]:
        try:
            ...
            # 1) 경로 기반 분석 (기존 로직은 조선일보용이지만 일부 공통 키워드 유지)
            if path:
