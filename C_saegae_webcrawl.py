#!/usr/bin/env python3
"""
URL 기반 뉴스 기사 크롤러 (세계일보 전용)
 - 세계일보 기사 페이지를 크롤링하도록 특화된 셀렉터/패턴 반영
 - 입력: 단일 URL 또는 URL 목록 파일
 - 출력: 표준출력(JSON Lines) 또는 --out 파일(JSON Lines)
 - 옵션: --save-db 사용 시 DB 저장(C_saegae_database_manager.db_manager 활용)
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('saegae_url_crawler.log', encoding='utf-8'),
...
# 세계일보 기본 URL 목록 (인자를 주지 않으면 이 목록을 사용)
...
DEFAULT_CATEGORY_URLS: List[str] = [
    'https://www.segye.com/news/politics', #정치
    'https://www.segye.com/news/society', #사회
    'https://www.segye.com/news/culture', #문화
    'https://www.segye.com/newsList/0101070000000', #전국
    'https://www.segye.com/newsList/0101120000000', #피플
    'https://www.segye.com/news/opinion' #오피니언
]
...
class UrlArticleCrawler:
    ...
    def _extract_title(self, soup: BeautifulSoup) -> Optional[str]:
        ...
        selectors = [
            ...
            '.article_title',  # 세계일보 기사 제목
            '.news_title',     # 세계일보 뉴스 제목
            '.title',          # 일반 제목
            'h1',              # H1 태그
        ]
    ...
    def _extract_article_links(self, soup: BeautifulSoup, current_url: str) -> List[str]:
        # 세계일보 기사 링크 패턴 위주로 수집
        patterns = [
            # 세계일보 기사 URL 패턴(대표적으로 newsView/숫자, 또는 view 경로)
            re.compile(r'/newsView/\d+', re.I),
            re.compile(r'/view/\d{4}/\d{2}/\d{2}/[A-Za-z0-9]+' , re.I),
            # 레거시 패턴도 일부 유지
        ...
        for a in soup.find_all('a', href=True):
            ...
            # 세계일보 도메인 확인
            if 'segye.com' in href or href.startswith('/'):
                for pat in patterns:
                    if pat.search(href):
                        full = self._normalize_url(current_url, href)
                        if full and full not in seen:
                            # 세계일보 기사 URL인지 추가 검증
                            if self._is_segye_article_url(full):
                                links.append(full)
                                seen.add(full)
                        break
        return links

    def _is_segye_article_url(self, url: str) -> bool:
        """세계일보 기사 URL인지 확인"""
        try:
            parsed = urlparse(url)
            if 'segye.com' not in parsed.netloc:
                return False
            ...
            # 세계일보 대표 기사 URL 패턴
            # 1) /newsView/숫자
            if re.match(r'^newsView/\d+$', path, re.I):
                return True
            # 2) /view/년/월/일/임의ID
            if re.match(r'^view/\d{4}/\d{2}/\d{2}/[A-Za-z0-9]+$', path, re.I):
                return True
    ...
    def _derive_category_name(self, category_url: str) -> Optional[str]:
        try:
            ...
            # 세계일보는 경로 기반으로 카테고리를 구분하므로 경로를 우선 분석
            if path:
                # 경로 기반 카테고리 매핑 (세계일보 URL 구조)
                path_map = {
                    'news/opinion': '오피니언',
                    'news/politics': '정치', 
                    'news/society': '사회',
                    'news/culture': '문화',
                    'newsList/0101070000000': '전국',
                    'newsList/0101120000000': '피플',
                }
...
def main():
    parser = argparse.ArgumentParser(description='세계일보 URL 기반 뉴스 기사 크롤러')
    ...
    if category_urls:
        ...
        if not urls and DEFAULT_CATEGORY_URLS:
            ...
            if not urls:
                logger.info('입력된 URL이 없어 기본 세계일보 URL로 실행합니다.')
