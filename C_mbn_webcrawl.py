...
"""
URL 기반 뉴스 기사 크롤러
 - 매일경제 기사 페이지를 크롤링하도록 특화된 패턴 반영
 - 입력: 단일 URL 또는 URL 목록 파일
 - 출력: 표준출력(JSON Lines) 또는 --out 파일(JSON Lines)
 - 옵션: --save-db 사용 시 DB 저장(db_manager 활용)
"""
...
try:
    from C_mbn_database_manager import db_manager
except Exception:
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('mbn_url_crawler.log', encoding='utf-8'),
...
# 매일경제 기본 URL 목록 (인자를 주지 않으면 이 목록을 사용)
...
DEFAULT_CATEGORY_URLS: List[str] = [
    'https://www.mbn.co.kr/news/date/', #전체
    'https://www.mbn.co.kr/news/politics/', #정치
    'https://www.mbn.co.kr/news/society/', #사회
    'https://www.mbn.co.kr/news/economy/', #경제
    'https://www.mbn.co.kr/news/world/', #국제
    'https://www.mbn.co.kr/news/culture/', #문화
    'https://www.mbn.co.kr/news/sports/', #스포츠
    'https://www.mbn.co.kr/news/entertain/', #연예
    'https://www.mbn.co.kr/news/life/', #생활/건강
    'https://www.mbn.co.kr/news/corp/', #기업in
    'https://www.mbn.co.kr/news/approaching/', #인기척
    'https://www.mbn.co.kr/news/fact/', #사실확인
    'https://www.mbn.co.kr/news/park-golf', #파크골프
]

# 카테고리별 한글명 매핑 (MBN)
CATEGORY_MAP = {
    'https://www.mbn.co.kr/news/economy/' : '경제',
    'https://www.mbn.co.kr/news/corp/' : '기업',
    'https://www.mbn.co.kr/news/society/' : '사회',
    'https://www.mbn.co.kr/news/world/' : '국제',
    'https://www.mbn.co.kr/news/politics/' : '정치',
    'https://www.mbn.co.kr/news/it/' : 'IT/과학',
    'https://www.mbn.co.kr/news/culture/' : '문화',
    'https://www.mbn.co.kr/news/sports/' : '스포츠',
    'https://www.mbn.co.kr/news/entertain/' : '연예',
    'https://www.mbn.co.kr/news/life/' : '생활/건강',
...
class UrlArticleCrawler:
    ...
    def _extract_content(self, soup: BeautifulSoup) -> Optional[str]:
        selectors = [
            ...
            # 특화 추가 셀렉터
            '.art-text', '.art_text', '.news-text', '.news_text',
            '.article-view', '.article_view', '.view-content', '.view_content'
        ]
        for sel in selectors:
    ...
    def _extract_article_links(self, soup: BeautifulSoup, current_url: str) -> List[str]:
        # MBN 기사 링크 패턴 위주로 수집
        patterns = [
            # MBN 기사 URL 패턴 (news/카테고리/숫자)
            re.compile(r'/news/[a-z-]+/\d+/?$', re.I),
            # 기타 섹션 하위의 숫자 ID 패턴
            re.compile(r'/opinion/\d+/?$', re.I),
        ]
        links: List[str] = []
        seen = set()
        for a in soup.find_all('a', href=True):
            ...
            # MBN 도메인 확인
            if 'mbn.co.kr' in href or href.startswith('/'):
                for pat in patterns:
                    if pat.search(href):
                        full = self._normalize_url(current_url, href)
                        if full and full not in seen:
                            # MBN 기사 URL인지 추가 검증
                            if self._is_mbn_article_url(full):
                                links.append(full)
                                seen.add(full)
                        break
        return links

    def _is_mbn_article_url(self, url: str) -> bool:
        """MBN 기사 URL인지 확인"""
        try:
            parsed = urlparse(url)
            if 'mbn.co.kr' not in parsed.netloc:
                return False
            ...
            if len(parts) >= 3 and parts[0] == 'news':
                if re.match(r'^[a-z-]+$', parts[1]) and re.match(r'^\d+$', parts[2]):
                    return True
    ...
    def _derive_category_name(self, category_url: str) -> Optional[str]:
        try:
            ...
            # MBN: /news/{카테고리}/ 형태의 경로 사용
            if path:
                parts = path.split('/')
                if len(parts) >= 2 and parts[0] == 'news':
                    category_segment = parts[1]
                    segment_map = {
                        'economy': '경제',
                        'corp': '기업',
                        'society': '사회',
                    ...
                    if category_segment in segment_map:
                        return segment_map[category_segment]
            
            # 오피니언은 /opinion/ 하위 (예비)
            if path.startswith('opinion'):
                return '오피니언'

            # 최종 fallback: CATEGORY_MAP의 키와 prefix 매칭
            for key_url, name in CATEGORY_MAP.items():
                if category_url.startswith(key_url):
                    return name
            return None
        except Exception:
            return None
...
def main():
    ...
    if category_urls:
        ...
        if not urls and DEFAULT_CATEGORY_URLS:
            ...
            if not urls:
                logger.info('입력된 URL이 없어 기본 매일경제 URL로 실행합니다.')
