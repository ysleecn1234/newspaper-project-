...
"""
RSS 기반 뉴스 기사 크롤러
 - MBN RSS 피드를 파싱하여 기사 정보를 추출하고 DB에 저장
 - 입력: RSS URL 또는 RSS URL 목록 파일
 - 출력: 표준출력(JSON Lines) 또는 --out 파일(JSON Lines)
 - 옵션: --save-db 사용 시 DB 저장(C_mbn_database_manager.db_manager 활용)
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('mbn_rss_crawler.log', encoding='utf-8', mode='w'),
...
DEFAULT_RSS_URLS: List[str] = [
    'https://www.mbn.co.kr/rss/', # 전체
    'https://www.mbn.co.kr/rss/politics/', # 정치
    'https://www.mbn.co.kr/rss/economy/', # 경제
    'https://www.mbn.co.kr/rss/society/', # 사회
    'https://www.mbn.co.kr/rss/international/', # 국제
    'https://www.mbn.co.kr/rss/culture/', # 문화
    'https://www.mbn.co.kr/rss/enter/', # 연예
    'https://www.mbn.co.kr/rss/sports/', # 스포츠
    'https://www.mbn.co.kr/rss/health/', # 생활/건강
]

# 카테고리별 한글명 매핑 (MBN RSS용)
RSS_CATEGORY_MAP = {
    'politics': '정치',
    'economy': '경제',
    'society': '사회',
...
class RSSArticleCrawler:
    ...
    def _extract_category_from_url(self, rss_url: str) -> Optional[str]:
        """RSS URL에서 카테고리명 추출 (MBN 규칙)

        예) https://www.mbn.co.kr/rss/politics/ -> '정치'
            https://www.mbn.co.kr/rss/ -> None
        """
        try:
            parsed = urlparse(rss_url)
            # '/rss/politics/' 형태에서 'politics' 추출
            segments = [seg for seg in parsed.path.split('/') if seg]
            category_key: Optional[str] = None
            if 'rss' in segments:
                idx = segments.index('rss')
                if idx + 1 < len(segments):
                    category_key = segments[idx + 1]
            # 매핑 후 반환
            return RSS_CATEGORY_MAP.get(category_key) if category_key else None
        except Exception:
            return None
...
def main():
    parser = argparse.ArgumentParser(description='MBN RSS 피드 크롤러')
    ...
    if not rss_urls:
        logger.info('입력된 RSS URL이 없어 기본 MBN RSS URL로 실행합니다.')
