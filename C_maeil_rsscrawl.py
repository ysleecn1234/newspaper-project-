...
"""
RSS 기반 뉴스 기사 크롤러
 - 매일경제 RSS 피드를 파싱하여 기사 정보를 추출하고 DB에 저장
 - 입력: RSS URL 또는 RSS URL 목록 파일
 - 출력: 표준출력(JSON Lines) 또는 --out 파일(JSON Lines)
 - 옵션: --save-db 사용 시 DB 저장(C_maeil_database_manager.db_manager 활용)
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('maeil_rss_crawler.log', encoding='utf-8', mode='w'),
...
DEFAULT_RSS_URLS: List[str] = [
    ...
    'https://www.mk.co.kr/rss/40300001/', #전체 뉴스
    ...
    'https://www.mk.co.kr/rss/40200124/', #MBA
    'https://www.mk.co.kr/rss/40200003/', #머니 앤 리치스
    'https://www.mk.co.kr/rss/50000001/', #이코노미 전체
    'https://www.mk.co.kr/rss/60000007/', #시티라이프 전체
]

# 카테고리별 한글명 매핑 (매일경제 RSS용: RSS 경로의 숫자 코드 기준)
RSS_CATEGORY_MAP = {
    '30000001': '헤드라인',
    '40300001': '전체 뉴스',
    '30200030': '정치',
...
class RSSArticleCrawler:
    ...
    def _extract_category_from_url(self, rss_url: str) -> Optional[str]:
        """RSS URL에서 카테고리명 추출 (트레일링 슬래시 및 쿼리 제거)"""
        # 쿼리 제거
        parsed = urlparse(rss_url)
        path = parsed.path.rstrip('/')
        # 마지막 세그먼트 추출 (예: /rss/30000001)
...
def main():
    parser = argparse.ArgumentParser(description='매일경제 RSS 피드 크롤러')
    ...
    if not rss_urls:
        logger.info('입력된 RSS URL이 없어 기본 매일경제 RSS URL로 실행합니다.')
