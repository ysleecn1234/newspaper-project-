...
try:
    from C_maeil_database_manager import db_manager
except Exception:
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('maeil_url_crawler.log', encoding='utf-8'),
...
DEFAULT_CATEGORY_URLS: List[str] = [
    'https://www.mk.co.kr/news/economy/', #경제
    'https://www.mk.co.kr/news/business/', #기업
    'https://www.mk.co.kr/news/society/', #사회
    'https://www.mk.co.kr/news/world/', #국제
    'https://www.mk.co.kr/news/realestate/', #부동산
    'https://www.mk.co.kr/news/stock/', #증권
    'https://www.mk.co.kr/news/politics/', #정치
    'https://www.mk.co.kr/news/it/', #IT/과학
    'https://www.mk.co.kr/news/culture/', #문화
    'https://www.mk.co.kr/opinion/', #오피니언
    'https://www.mk.co.kr/news/sports/', #스포츠
]
...
class UrlArticleCrawler:
    ...
    def _extract_article_links(self, soup: BeautifulSoup, current_url: str) -> List[str]:
        # 매일경제 기사 링크 패턴 위주로 수집
        patterns = [
            # 매일경제 기사 URL 패턴 (news/카테고리/숫자)
            re.compile(r'/news/[a-z-]+/\d+/?$', re.I),
            # 매일경제 오피니언 등 섹션 하위의 숫자 ID 패턴
        ...
        for a in soup.find_all('a', href=True):
            ...
            # 매일경제 도메인 확인
            if 'mk.co.kr' in href or href.startswith('/'):
                for pat in patterns:
                    if pat.search(href):
                        full = self._normalize_url(current_url, href)
                        if full and full not in seen:
                            # 매일경제 기사 URL인지 추가 검증
                            if self._is_mk_article_url(full):
                                links.append(full)
                                seen.add(full)
                        break
        return links

    def _is_mk_article_url(self, url: str) -> bool:
        """매일경제 기사 URL인지 확인"""
        try:
            parsed = urlparse(url)
            if 'mk.co.kr' not in parsed.netloc:
                return False
