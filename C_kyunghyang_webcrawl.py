...
"""
URL 기반 뉴스 기사 크롤러
 - 경향신문 기사 페이지를 크롤링하도록 특화된 셀렉터/패턴 반영
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('kyunghyang_url_crawler.log', encoding='utf-8'),
...
# 경향신문 기본 URL 목록 (인자를 주지 않으면 이 목록을 사용)
...
DEFAULT_CATEGORY_URLS: List[str] = [
    'https://www.khan.co.kr/opinion', #오피니언
    'https://www.khan.co.kr/politics', #정치
    'https://www.khan.co.kr/economy', #경제
    'https://www.khan.co.kr/national', #사회
    'https://www.khan.co.kr/world', #국제
    'https://www.khan.co.kr/culture', #문화
    'https://www.khan.co.kr/life', #라이프
]
...
class UrlArticleCrawler:
    ...
    def _extract_article_links(self, soup: BeautifulSoup, current_url: str) -> List[str]:
        # 경향신문 기사 링크 패턴 위주로 수집
        patterns = [
            # 경향신문 기사 URL 패턴 (카테고리/년/월/일/고유ID)
            re.compile(r'/[a-z-]+/\d{4}/\d{2}/\d{2}/[A-Z0-9]+/?$', re.I),
            # 경향신문 기사 URL 패턴 (카테고리/고유ID)
        ...
        for a in soup.find_all('a', href=True):
            ...
            # 경향신문 도메인 확인
            if 'khan.co.kr' in href or href.startswith('/'):
                for pat in patterns:
                    if pat.search(href):
                        full = self._normalize_url(current_url, href)
                        if full and full not in seen:
                            # 경향신문 기사 URL인지 추가 검증
                            if self._is_khan_article_url(full):
                                links.append(full)
                                seen.add(full)
                        break
        return links

    def _is_khan_article_url(self, url: str) -> bool:
        """경향신문 기사 URL인지 확인"""
        try:
            parsed = urlparse(url)
            if 'khan.co.kr' not in parsed.netloc:
                return False
            ...
            # 경향신문 기사 URL 패턴: 카테고리/년/월/일/고유ID 또는 카테고리/고유ID
            parts = path.split('/')
            if len(parts) >= 2:
    ...
    def _derive_category_name(self, category_url: str) -> Optional[str]:
        try:
            ...
            # 경향신문은 경로 기반으로 카테고리를 구분하므로 경로를 우선 분석
            if path:
                # 경로 기반 카테고리 매핑 (경향신문 URL 구조)
                path_map = {
                    'economy': '경제',
                    'politics': '정치', 
                    'national': '사회',
...
def main():
    ...
    if category_urls:
        ...
        if not urls and DEFAULT_CATEGORY_URLS:
            ...
            if not urls:
                logger.info('입력된 URL이 없어 기본 경향신문 URL로 실행합니다.')
