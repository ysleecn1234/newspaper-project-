...
"""
URL 기반 뉴스 기사 크롤러
 - 문화일보 기사 페이지를 크롤링하도록 특화된 셀렉터/패턴 반영
 - 입력: 단일 URL 또는 URL 목록 파일
 - 출력: 표준출력(JSON Lines) 또는 --out 파일(JSON Lines)
 - 옵션: --save-db 사용 시 DB 저장(C_munhwa_database_manager.db_manager 활용)
"""
...
try:
    from C_munhwa_database_manager import db_manager
except Exception:
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('munhwa_url_crawler.log', encoding='utf-8'),
...
# 문화일보 기본 URL 목록 (인자를 주지 않으면 이 목록을 사용)
...
DEFAULT_CATEGORY_URLS: List[str] = [
    'https://www.munhwa.com/opinion', #오피니언
    'https://www.munhwa.com/politics', #정치
    'https://www.munhwa.com/economy', #경제
    'https://www.munhwa.com/society', #사회
    'https://www.munhwa.com/world', #국제
    'https://www.munhwa.com/culture', #문화
    'https://www.munhwa.com/enter', #연예
    'https://www.munhwa.com/sports', #스포츠
    'https://www.munhwa.com/people', #피플
]
...
class UrlArticleCrawler:
    ...
    def _extract_content(self, soup: BeautifulSoup) -> Optional[str]:
        selectors = [
            ...
            # 문화일보 뉴스 본문 셀렉터
            '.news_view', '.news-view', '.view_news', '.view-news'
        ]
        for sel in selectors:
    ...
    def _extract_article_links(self, soup: BeautifulSoup, current_url: str) -> List[str]:
        # 문화일보 기사 링크 패턴 위주로 수집
        patterns = [
            # 문화일보 기사 URL 패턴 (카테고리/년/월/일/고유ID)
            re.compile(r'/[a-z-]+/\d{4}/\d{2}/\d{2}/[A-Z0-9]+/?$', re.I),
            # 문화일보 기사 URL 패턴 (카테고리/고유ID)
            re.compile(r'/[a-z-]+/[A-Z0-9]+/?$', re.I),
            # 문화일보 기사 URL 패턴 (뉴스/카테고리/고유ID)
        ...
        for a in soup.find_all('a', href=True):
            ...
            # 문화일보 도메인 확인
            if 'munhwa.com' in href or href.startswith('/'):
                for pat in patterns:
                    if pat.search(href):
                        full = self._normalize_url(current_url, href)
                        if full and full not in seen:
                            # 문화일보 기사 URL인지 추가 검증
                            if self._is_munhwa_article_url(full):
                                links.append(full)
                                seen.add(full)
                        break
        return links

    def _is_munhwa_article_url(self, url: str) -> bool:
        """문화일보 기사 URL인지 확인"""
        try:
            parsed = urlparse(url)
            if 'munhwa.com' not in parsed.netloc:
                return False
            ...
            # 문화일보 기사 URL 패턴: 카테고리/년/월/일/고유ID 또는 카테고리/고유ID 또는 뉴스/카테고리/고유ID
            parts = path.split('/')
            if len(parts) >= 2:
                # 마지막 부분이 고유ID인지 확인 (숫자 또는 대문자+숫자 조합)
                if re.match(r'^[A-Z0-9]+$', parts[-1]) or re.match(r'^\d+$', parts[-1]):
                    # 년/월/일 형식이 있는 경우 (5개 부분)
                    if len(parts) >= 5:
                        if (re.match(r'^\d{4}$', parts[-4]) and 
                            re.match(r'^\d{2}$', parts[-3]) and 
                            re.match(r'^\d{2}$', parts[-2])):
                            return True
                    # 뉴스/카테고리/고유ID 형식 (3개 부분)
                    elif len(parts) == 3 and parts[0] == 'news':
                        return True
                    # 카테고리/고유ID 형식 (2개 부분)
                    elif len(parts) == 2:
                        return True
    ...
    def _derive_category_name(self, category_url: str) -> Optional[str]:
        try:
            ...
            # 문화일보는 경로 기반으로 카테고리를 구분하므로 경로를 우선 분석
            if path:
                # 경로 기반 카테고리 매핑 (문화일보 URL 구조)
                path_map = {
                    'economy': '경제',
                    'politics': '정치', 
                    'society': '사회',
            ...
            tail_map = {
                ...
                'national': '사회', 'soc': '사회',
                'world': '국제', 'int': '국제',
                'culture': '문화', 'entertainment': '문화',
                'opinion': '오피니언', 'op': '오피니언',
                'life': '라이프', 'lif': '라이프',
                'sports': '스포츠', 'sport': '스포츠', 'spo': '스포츠',
                'science': '과학/환경', 'sci': '과학/환경',
                'people': '사람', 'peo': '사람',
                'local': '지역', 'loc': '지역',
            }
...
def main():
    ...
    if category_urls:
        ...
        if not urls and DEFAULT_CATEGORY_URLS:
            ...
            if not urls:
                logger.info('입력된 URL이 없어 기본 문화일보 URL로 실행합니다.')
